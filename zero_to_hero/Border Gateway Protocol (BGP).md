...
1. [[Border Gateway Protocol (BGP)#Border Gateway Protocol (BGP)|Border Gateway Protocol (BGP)]]
	1. [[Border Gateway Protocol (BGP)##BGP Overview|BGP Overview]]
		1. [[Border Gateway Protocol (BGP)###Purpose and Function|Purpose and Function]]
		2. [[Border Gateway Protocol (BGP)###Autonomous System Numbers (ASNs)|Autonomous System Numbers (ASNs)]]
		3. [[Border Gateway Protocol (BGP)###Transport|Transport]]
	2. [[Border Gateway Protocol (BGP)##BGP Components|BGP Components]]
		1. [[Border Gateway Protocol (BGP)###BGP Speaker or Peer|BGP Speaker or Peer]]
		2. [[Border Gateway Protocol (BGP)###BGP Session|BGP Session]]
	3. [[Border Gateway Protocol (BGP)##BGP Session Types|BGP Session Types]]
		1. [[Border Gateway Protocol (BGP)###Internal BGP (iBGP)|Internal BGP (iBGP)]]
		2. [[Border Gateway Protocol (BGP)###External BGP (eBGP)|External BGP (eBGP)]]
	4. [[Border Gateway Protocol (BGP)##AS Types|AS Types]]
		1. [[Border Gateway Protocol (BGP)###Stub AS|Stub AS]]
		2. [[Border Gateway Protocol (BGP)###Multihomed AS|Multihomed AS]]
		3. [[Border Gateway Protocol (BGP)###Transit AS|Transit AS]]
	5. [[Border Gateway Protocol (BGP)##Split Horizon|Split Horizon]]
	6. [[Border Gateway Protocol (BGP)##Route Reflectors|Route Reflectors]]
		1. [[Border Gateway Protocol (BGP)###The Problem Route Reflectors Solve|The Problem Route Reflectors Solve]]
			1. [[Border Gateway Protocol (BGP)####What this means:|What this means:]]
			2. [[Border Gateway Protocol (BGP)####Why this is a problem:|Why this is a problem:]]
		2. [[Border Gateway Protocol (BGP)###What Route Reflectors Do|What Route Reflectors Do]]
		3. [[Border Gateway Protocol (BGP)###Route Reflector Clients|Route Reflector Clients]]
			1. [[Border Gateway Protocol (BGP)####How it works:|How it works:]]
		4. [[Border Gateway Protocol (BGP)###Route Reflector Clusters|Route Reflector Clusters]]
			1. [[Border Gateway Protocol (BGP)####Simple Example|Simple Example]]
			2. [[Border Gateway Protocol (BGP)####Without RRs:|Without RRs:]]
			3. [[Border Gateway Protocol (BGP)####With a Route Reflector:|With a Route Reflector:]]
			4. [[Border Gateway Protocol (BGP)####Multi-Cluster Example|Multi-Cluster Example]]
	7. [[Border Gateway Protocol (BGP)##RIBs (Routing Information Bases)|RIBs (Routing Information Bases)]]
		1. [[Border Gateway Protocol (BGP)###1. RIB-in — “Everything I heard”|1. RIB-in — “Everything I heard”]]
		2. [[Border Gateway Protocol (BGP)###2. Local RIB — “What I decide to keep”|2. Local RIB — “What I decide to keep”]]
		3. [[Border Gateway Protocol (BGP)###3. RIB-out — “What I choose to send out”|3. RIB-out — “What I choose to send out”]]
		4. [[Border Gateway Protocol (BGP)###Putting It All Together|Putting It All Together]]
	8. [[Border Gateway Protocol (BGP)##BGP Attributes|BGP Attributes]]
		1. [[Border Gateway Protocol (BGP)###Four Types of BGP Attributes|Four Types of BGP Attributes]]
		2. [[Border Gateway Protocol (BGP)###1. Well-Known Mandatory Attributes|1. Well-Known Mandatory Attributes]]
		3. [[Border Gateway Protocol (BGP)###2. Well-Known Discretionary Attributes|2. Well-Known Discretionary Attributes]]
		4. [[Border Gateway Protocol (BGP)###3. Optional Transitive Attributes|3. Optional Transitive Attributes]]
		5. [[Border Gateway Protocol (BGP)###4. Optional Non-Transitive Attributes|4. Optional Non-Transitive Attributes]]
		6. [[Border Gateway Protocol (BGP)###Key Attributes|Key Attributes]]
			1. [[Border Gateway Protocol (BGP)####AS_PATH (Well-known mandatory)|AS_PATH (Well-known mandatory)]]
			2. [[Border Gateway Protocol (BGP)####NEXT_HOP (Well-known mandatory)|NEXT_HOP (Well-known mandatory)]]
			3. [[Border Gateway Protocol (BGP)####ORIGIN(Well-known mandatory)|ORIGIN(Well-known mandatory)]]
			4. [[Border Gateway Protocol (BGP)####LOCAL_PREF (Well-known discretionary)|LOCAL_PREF (Well-known discretionary)]]
				1. [[Border Gateway Protocol (BGP)#####Where LOCAL_PREF Stands in the Decision Process|Where LOCAL_PREF Stands in the Decision Process]]
			5. [[Border Gateway Protocol (BGP)####MED — Multi Exit Discriminator (Optional non-transitive)|MED — Multi Exit Discriminator (Optional non-transitive)]]
				1. [[Border Gateway Protocol (BGP)#####Real-World Example (The Classic Use Case)|Real-World Example (The Classic Use Case)]]
				2. [[Border Gateway Protocol (BGP)#####Very Important Things to Know About MED|Very Important Things to Know About MED]]
				3. [[Border Gateway Protocol (BGP)#####Quick Comparison: The Big Three Traffic Engineering Tools|Quick Comparison: The Big Three Traffic Engineering Tools]]
			6. [[Border Gateway Protocol (BGP)####COMMUNITY (Optional transitive)|COMMUNITY (Optional transitive)]]
				1. [[Border Gateway Protocol (BGP)#####The Most Famous Communities (Used by Almost Every ISP)|The Most Famous Communities (Used by Almost Every ISP)]]
				2. [[Border Gateway Protocol (BGP)#####Quick Facts Table|Quick Facts Table]]
	9. [[Border Gateway Protocol (BGP)##Route Selection|Route Selection]]
		1. [[Border Gateway Protocol (BGP)###BGP Route Selection|BGP Route Selection]]
			1. [[Border Gateway Protocol (BGP)####1. Highest weight|1. Highest weight]]
			2. [[Border Gateway Protocol (BGP)####2. Highest local preference (LOCAL_PREF)|2. Highest local preference (LOCAL_PREF)]]
			3. [[Border Gateway Protocol (BGP)####3. Prefer routes that originated locally|3. Prefer routes that originated locally]]
			4. [[Border Gateway Protocol (BGP)####4. Shortest AS path|4. Shortest AS path]]
			5. [[Border Gateway Protocol (BGP)####5. Lowest origin type|5. Lowest origin type]]
			6. [[Border Gateway Protocol (BGP)####6. Lowest MED (Multi-Exit Discriminator)|6. Lowest MED (Multi-Exit Discriminator)]]
			7. [[Border Gateway Protocol (BGP)####7. Lowest IGP metric to the NEXT_HOP|7. Lowest IGP metric to the NEXT_HOP]]
			8. [[Border Gateway Protocol (BGP)####8. Prefer EBGP routes over IBGP routes|8. Prefer EBGP routes over IBGP routes]]
			9. [[Border Gateway Protocol (BGP)####9. If ECMP is enabled: install up to 10 equal-cost routes|9. If ECMP is enabled: install up to 10 equal-cost routes]]
			10. [[Border Gateway Protocol (BGP)####10. Lowest router ID|10. Lowest router ID]]
				1. [[Border Gateway Protocol (BGP)#####Easy Real-Life Analogy|Easy Real-Life Analogy]]
		2. [[Border Gateway Protocol (BGP)###Quick Summary (Super Simple)|Quick Summary (Super Simple)]]
	10. [[Border Gateway Protocol (BGP)##FortiGate BGP Implementation|FortiGate BGP Implementation]]
		1. [[Border Gateway Protocol (BGP)###1. Scaling Capabilities|1. Scaling Capabilities]]
			1. [[Border Gateway Protocol (BGP)####What this means:|What this means:]]
		2. [[Border Gateway Protocol (BGP)###2.  FortiGate Does Not Advertise Anything by Default|2.  FortiGate Does Not Advertise Anything by Default]]
			1. [[Border Gateway Protocol (BGP)####Why?|Why?]]
			2. [[Border Gateway Protocol (BGP)####How do you make it advertise routes?|How do you make it advertise routes?]]
			3. [[Border Gateway Protocol (BGP)####Option A: Redistribute routes into BGP|Option A: Redistribute routes into BGP]]
			4. [[Border Gateway Protocol (BGP)####Option B: Use the network command|Option B: Use the network command]]
			5. [[Border Gateway Protocol (BGP)####❗Rule:|❗Rule:]]
			6. [[Border Gateway Protocol (BGP)####3.  FortiGate Accepts All Routes by Default|3.  FortiGate Accepts All Routes by Default]]
			7. [[Border Gateway Protocol (BGP)####How Do You Control What You Accept?|How Do You Control What You Accept?]]
		3. [[Border Gateway Protocol (BGP)###SUMMARY|SUMMARY]]
	11. [[Border Gateway Protocol (BGP)##BGP Route Filtering Options (Access Lists vs. Prefix Lists vs. Route Maps)|BGP Route Filtering Options (Access Lists vs. Prefix Lists vs. Route Maps)]]
		1. [[Border Gateway Protocol (BGP)###Access Lists (basic IP filter)|Access Lists (basic IP filter)]]
		2. [[Border Gateway Protocol (BGP)###Prefix Lists (better for networks)|Prefix Lists (better for networks)]]
		3. [[Border Gateway Protocol (BGP)###Route Maps (the Swiss Army knife)|Route Maps (the Swiss Army knife)]]
		4. [[Border Gateway Protocol (BGP)###Quick comparison table (exactly what the slide says, but in plain English)|Quick comparison table (exactly what the slide says, but in plain English)]]
	12. [[Border Gateway Protocol (BGP)##BGP Commands Comparison on FortiGate|BGP Commands Comparison on FortiGate]]
		1. [[Border Gateway Protocol (BGP)###Quick cheat-sheet (every FortiGate BGP admin has this memorized)|Quick cheat-sheet (every FortiGate BGP admin has this memorized)]]
	13. [[Border Gateway Protocol (BGP)##Use Case 1 – Configure ECMP With BGP Routes on FortiGate|Use Case 1 – Configure ECMP With BGP Routes on FortiGate]]
